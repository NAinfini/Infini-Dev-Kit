...
export type { SpringProfile } from "@infini-dev-kit/theme-core";
...
export type MotionSafeProps<T extends ElementType> = Omit<ComponentPropsWithoutRef<T>, MotionConflictingProps>;
...
export interface LayeredCardProps extends Omit<MotionSafeProps<"div">, "children"> {
  ...
  /** When true, renders skeleton instead of layers */
  loading?: boolean;
}
...
export interface FlipCardProps extends Omit<MotionSafeProps<"div">, "children" | "onClick"> {
  ...
  clickToFlip?: boolean;
  /** When true, renders skeleton instead of content */
  loading?: boolean;
}

// ── Buttons ──

export type DepthButtonType = "primary" | "secondary" | "danger" | "success" | "warning" | "info";
...
export interface DepthButtonProps extends Omit<MotionSafeProps<"button">, "children" | "onClick" | "type" | "disabled" | "color"> {
  ...
  ripple?: boolean;
  /** Enable pointer-position-aware 3D tilt on hover (default: false) */
  hoverTilt?: boolean;
  /** Tilt pressure intensity — higher = more skew (default: 1) */
  hoverPressure?: number;
  disabled?: boolean;
  size?: "sm" | "md" | "lg";
  /** Effect overlays rendered inside the button */
...
export interface SoftClayButtonProps extends Omit<MotionSafeProps<"button">, "children" | "type" | "disabled" | "color"> {
  children: ReactNode;
  disabled?: boolean;
  size?: "sm" | "md" | "lg";
  /** Clay background color (default: theme primary) */
  color?: string;
}

export interface LiquidFillButtonProps extends Omit<MotionSafeProps<"button">, "children" | "type" | "disabled" | "color"> {
  children: ReactNode;
  disabled?: boolean;
  size?: "sm" | "md" | "lg";
  /** Resting background color */
  color?: string;
  /** Fill color on hover (default: theme primary) */
  fillColor?: string;
}

export interface CrystalPrismButtonProps extends Omit<MotionSafeProps<"button">, "children" | "type" | "disabled" | "color"> {
  children: ReactNode;
  disabled?: boolean;
  size?: "sm" | "md" | "lg";
  /** Backdrop blur in px (default: 72) */
  blur?: number;
  /** Background tint color override */
  tint?: string;
}

export interface SlideRevealButtonProps extends Omit<MotionSafeProps<"button">, "children" | "type" | "disabled" | "color"> {
  children: ReactNode;
  disabled?: boolean;
  size?: "sm" | "md" | "lg";
  /** Icon ReactNode that slides in on hover */
  icon: ReactNode;
  /** Background color (default: theme success) */
  color?: string;
}
...
export type EffectRevealMode = "hover" | "always";
export type LiquidFillMode = "bottom-up" | "left-to-right" | "both-sides" | "diagonal";

export interface SpectrumBorderEffectProps extends Omit<MotionSafeProps<"div">, "children"> {
  children: ReactNode;
  /** Border palette. Defaults to a theme-aware rainbow mix. */
  colors?: string[];
  /** Rotate the visible border segment when full motion is enabled (default: true) */
  rotate?: boolean;
  /** Whether the border is always visible or only revealed on hover (default: "hover") */
  revealMode?: EffectRevealMode;
  /** Visible conic segment in degrees. Use 360 for a full border (default: 150) */
  visibleArc?: number;
  /** Border width in px (default: 2) */
  borderWidth?: number;
  /** Host-driven glow multiplier (default: 1) */
  glowIntensity?: number;
  /** Border radius override (default: theme radius) */
  radius?: number | string;
  /** CSS display mode for the wrapper (default: "inline-flex") */
  display?: CSSProperties["display"];
}

export interface LiquidFillEffectProps extends Omit<MotionSafeProps<"div">, "children" | "color"> {
  children: ReactNode;
  /** Fill color. Defaults to theme primary. */
  color?: string;
  /** Entry direction / pattern (default: "bottom-up") */
  mode?: LiquidFillMode;
  /** Whether the fill is always visible or only revealed on hover (default: "hover") */
  revealMode?: EffectRevealMode;
  /** Fill overlay opacity 0–1 (default: 0.34) */
  opacity?: number;
  /** Border radius override (default: theme radius) */
  radius?: number | string;
  /** CSS display mode for the wrapper (default: "inline-flex") */
...
export interface ShimmerSweepEffectProps extends Omit<MotionSafeProps<"div">, "children"> {
  children: ReactNode;
  /** Sweep band tint (default: translucent white) */
  sweepColor?: string;
  /** Under-glow tint (default: theme glow color) */
  glowColor?: string;
  /** Whether the shimmer is always visible or only triggered on hover (default: "hover") */
  revealMode?: EffectRevealMode;
  /** Hover lift in px when full motion is enabled (default: 6) */
  lift?: number;
  /** Glow multiplier (default: 1) */
  glowIntensity?: number;
  /** Sweep angle in degrees (default: -13) */
  angle?: number;
...
/** Shared error-handling contract for P0 components. */
export interface ErrorHandlingProps {
  /** Current error — renders errorContent or default error UI when set */
  error?: Error | string;
  /** Callback when a rendering or callback failure occurs */
  onError?: (error: Error) => void;
  /** Custom error UI — replaces default error message */
  errorContent?: ReactNode;
}

/** Shared loading-state contract for components with async content. */
export interface LoadingProps {
  /** When true, renders loadingContent or a default skeleton */
  loading?: boolean;
  /** Custom loading UI — replaces default skeleton */
  loadingContent?: ReactNode;
}

/** Shared empty-state contract for data components. */
export interface EmptyStateProps {
  /** Content shown when the component has no data */
  emptyContent?: ReactNode;
}

// ── Overlay System ──

/** Config for each overlay effect that can be injected into a button */
export type OverlayConfig =
  | ({ effect: "spectrum-border" } & SpectrumBorderOverlay)
  | ({ effect: "liquid-fill" } & LiquidFillOverlay)
  | ({ effect: "shimmer-sweep" } & ShimmerSweepOverlay)
  | ({ effect: "spotlight-glow" } & SpotlightGlowOverlay)
  | ({ effect: "laser-glow" } & LaserGlowOverlay)
  | ({ effect: "cosmic-glow" } & CosmicGlowOverlay)
  | ({ effect: "glitch-glow" } & GlitchGlowOverlay)
  | ({ effect: "neon-scanlines" } & NeonScanlinesOverlay)
  | ({ effect: "ripple" } & RippleOverlay)
  | ({ effect: "directional-reveal" } & DirectionalRevealOverlay)
  | ({ effect: "glitch" } & GlitchOverlayConfig);

export interface SpectrumBorderOverlay {
  colors?: string[];
  rotate?: boolean;
  revealMode?: EffectRevealMode;
  borderWidth?: number;
...
export interface LiquidFillOverlay {
  color?: string;
  revealMode?: EffectRevealMode;
  opacity?: number;
}

export interface ShimmerSweepOverlay {
  sweepColor?: string;
  revealMode?: EffectRevealMode;
  angle?: number;
  duration?: number;
...
export interface SpotlightGlowOverlay {
  glowColor?: string;
  glowIntensity?: number;
  glowRadius?: number;
}

export interface LaserGlowOverlay {
  glowColor?: string;
  glowIntensity?: number;
  spinSpeed?: number;
}

export interface CosmicGlowOverlay {
  glowColor?: string;
  glowIntensity?: number;
  glowRadius?: number;
}

export interface GlitchGlowOverlay {
  glowColor?: string;
  glowIntensity?: number;
  glowRadius?: number;
}

export interface NeonScanlinesOverlay {
  neonColor?: string;
  scanlines?: boolean;
  cornerClips?: boolean;
}
...
export interface DirectionalRevealOverlay {
  revealContent: ReactNode;
  direction?: "up" | "down" | "left" | "right";
  duration?: number;
}

export interface GlitchOverlayConfig {
  intensity?: "subtle" | "medium" | "heavy";
...
/** Context passed from the host button to overlay renderers */
...
/** Result from an overlay renderer */
export interface OverlayResult {
  /** Decoration nodes to render inside the button (before content) */
