...
export type InfiniSplitViewProps = {
  ...
  /** Auto-collapse to stacked layout below this breakpoint */
  collapseBelow?: "mobile" | "tablet";
  className?: string;
  style?: CSSProperties;
};
...
export const InfiniSplitView = forwardRef<HTMLDivElement, InfiniSplitViewProps>(
  function InfiniSplitView({
    ...
    const collapsed = collapseBelow === "mobile" ? isMobile
      : collapseBelow === "tablet" ? isTablet
      : false;
    ...
    if (collapsed) {
      return (
        <div
          ref={(node) => { mergedRef(node); }}
          className={className}
          style={{ display: "flex", flexDirection: "column", width: "100%", height: "100%", overflow: "auto", ...style }}
          {...rest}
        >
          <div>{left}</div>
          <div>{right}</div>
