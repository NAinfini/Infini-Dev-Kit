...
import type { SocialButtonProps, SocialPlatform, SocialSharePayload } from "../../motion-types";
...
export const SocialButton = forwardRef<HTMLButtonElement, SocialButtonProps>(function SocialButton({
  ...
  if (href && !sharer && !isDisabled) {
    ...
    if (!motionAllowed) {
      return (
        <a href={href} target="_blank" rel="noopener noreferrer" className={clsx(className)} style={mergedStyle} aria-label={rest["aria-label"] ?? displayLabel}>
          {content}
        </a>
      );
    }
    // REDUCED: opacity hover only
    if (!fullMotion) {
      return (
        <motion.a
          ref={ref as any}
          ...
          style={mergedStyle}
          aria-label={rest["aria-label"] ?? displayLabel}
  ...
  if (!fullMotion) {
    return (
      <motion.button
        ...
        aria-busy={isLoading || undefined}
        style={mergedStyle}
        whileHover={!isDisabled ? { opacity: 0.85 } : undefined}
        transition={transition}
      >
        {content}
      </motion.button>
