...
import type { GlitchOverlayProps } from "../../../motion-types";
...
export const GlitchOverlay = forwardRef<HTMLDivElement, GlitchOverlayProps>(
  ...
  // REDUCED: simple opacity flicker only, no procedural glitch layers
  if (!isAnimated) {
    return (
      <motion.div
        ref={ref}
        className={clsx(className)}
        style={{ position: "relative", display, ...style } as CSSProperties}
        {...rest}
        {...(trigger === "hover" ? { onMouseEnter: () => setBurstActive(true), onMouseLeave: () => setBurstActive(false) } : {})}
        {...(trigger === "click" ? { onClick: () => setBurstActive((v) => !v) } : {})}
        animate={
          (burstActive || trigger === "always")
            ? { opacity: [1, 0.88, 1, 0.92, 1] }
            : { opacity: 1 }
        }
        transition={{ duration: 0.3, ease: "easeInOut", repeat: trigger === "always" ? Infinity : 0 }}
      >
        {children}
      </motion.div>
    );
