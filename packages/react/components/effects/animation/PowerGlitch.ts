/**
 * Adapted from `7PH/powerglitch` (MIT): https://github.com/7PH/powerglitch
 */
...
const generateGlitchSliceLayer = (options: PowerGlitchOptions) => {
    ...
    const steps: LayerDefinition["steps"] = [];
    for (let index = 0; index < stepCount; ++ index) {
        ...
        const styles: LayerDefinition["steps"][number] = {
            opacity: '1',
...
const prepareGlitchElement = (element: HTMLElement, options: PowerGlitchOptions): { glitched: HTMLElement, container: HTMLDivElement, layersContainer: HTMLDivElement } => {
    ...
    if (! element.dataset.glitched) {
        ...
        layersContainer.insertBefore(element, layersContainer.firstChild);
        return {
            container,
            layersContainer,
            glitched: element,
