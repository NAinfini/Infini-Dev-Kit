...
import type { DepthButtonType } from "../../theme/motion-types";
...
function resolveTypeColors(
  ...
  switch (type) {
    ...
    case "danger":
      return {
        bg: `color-mix(in srgb, ${theme.palette.danger} 85%, ${theme.foundation.surface})`,
        shadow: `color-mix(in srgb, ${theme.palette.danger} 60%, black)`,
      };
    case "success":
      return {
        bg: `color-mix(in srgb, ${theme.palette.success} 85%, ${theme.foundation.surface})`,
        shadow: `color-mix(in srgb, ${theme.palette.success} 60%, black)`,
      };
    case "primary":
    default:
      return {
 ...
 */
export const DepthToggle = forwardRef<HTMLButtonElement, DepthToggleProps>(function DepthToggle({
  children,
  pressed,
  onToggle,
  ...
  // Pressed state uses primary colors; unpressed uses secondary
  const activeBg = pressed ? bg : (color ?? resolveTypeColors("secondary", theme).bg);
  const activeTextColor = pressed
    ? (buttonType === "secondary" ? theme.palette.text : theme.foundation.background)
    : theme.palette.text;
  ...
  const contentEl = (
    <>
      ...
      {/* Indicator light — bottom edge of button surface */}
      <span
        aria-hidden
        style={{
          position: "absolute",
          ...
          background: pressed ? "#22c55e" : `color-mix(in srgb, ${theme.palette.text} 15%, transparent)`,
