...
export const Terminal = forwardRef<HTMLDivElement, TerminalProps>(
  function Terminal({
    ...
    if (!motionAllowed) {
      return (
        <TerminalShell ref={ref} theme={theme} title={title} className={className} style={style} {...rest}>
          {commands.map((cmd, i) => (
            <div key={i}>
...
const TerminalShell = forwardRef<HTMLDivElement, {
  ...
  style?: CSSProperties;
  children: ReactNode;
}>(function TerminalShell({ theme, title, className, style, children, ...rest }, ref) {
  return (
    <div
      ref={ref}
      className={className}
